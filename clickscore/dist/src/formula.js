        ...
        function reload(){
          ...
          selectFaq.addEventListener('change', () => {
            ...
            if (selectFaq.value == 'faq_ligi') {
                faqExpl.innerHTML += `Wybierając opcje "Select League" można wybrać skład zespołów występujących w Grand Prix w danym sezonie. Po wyborze składów wyświetlają się zawodnicy w nich występujący. <br>
                Do każdego zawodnika przypisane są umiejętności kolejno:<br>
                Tec - technika<br>
                Con - kondycja<br>
                Eng - silnik<br>
            ...
            if (selectFaq.value == 'faq_yourL') {
                faqExpl.innerHTML += `W grze możesz także stworzyć własnych zawodników i zespoły. W opcji "Select League" wybierając opcję "Your Teams" wyświetli się do wyboru ilu zawodników mają wziac udział w GP.<br>
            ...
            if (selectFaq.value == 'faq_skills') {
                faqExpl.innerHTML += `Umiejętności drużyn to najważniejszy element rozgrywki aby określić siłę danego zawodnika. W symulatorze formuły mamy do wybrania takie skille: <br>
                Tec - technika (odpowiada za techniczne przygotowanie danego zawodnika)<br>
                Con - kondycja (ma znaczenie podczas trwania zawodów. Im lepsza tym zawodnik dłużej wykorzystuje pełnie swoich możliwości)<br>
                Eng - silnik (odpowiada za to jak mocny pojazd przygotowali zawodnikowi inżynierzy zespołu)<br> `
            }
            
            if (selectFaq.value == 'faq_plays') {
                faqExpl.innerHTML += `
                Podczas sezonu odbywa sie kilkanaście Grand Prix o różnych ilościach okrążeń. Po rozpoczęciu wyścigu za pomocą "Race Start" pojawi się klasyfikacja danego wyścigu według zajętych miejsc.<br>Po każdym wyścigu zawodnicy są klasyfikowani wg schematu punktów obowiązujących w F1<br>
                1 miejsce - 25 pkt<br>
                2 miejsce - 18 pkt<br>
                3 miejsce - 15 pkt<br>
                4 miejsce - 12 pkt<br>
          ...
          select.addEventListener('change', () => {
            ...
            if (select.value == 'f12018') {
                riders.splice(0, riders.length);
                const team_qty = f1_18.length;
                showTeam.innerHTML += `<input type="button" value="${team_qty}" id="team_qty">
                <input type="text" placeholder="Team" readonly>
                <input type="number" placeholder="Tec" readonly>
                <input type="number" placeholder="Con" readonly>
                ...
                for (let i = 0; i < team_qty; i++) {
                    ...
                    showTeam.innerHTML += `<tr>
               <td> <input type="text" value="${f1_18[i].name}" id="rider${i}_name"> </td>
               <td> <input type="text" value="${f1_18[i].team}" id="rider${i}_team"> </td>
               <td><input type="number" value="${f1_18[i].tech}" id="rider${i}_tech" max="99" min="1"> </td>
                <td><input type="number" value="${f1_18[i].con}" id="rider${i}_con" max="99" min="1"> </td>
                    <td><input type="number" value="${f1_18[i].eng}" id="rider${i}_eng" max="99" min="1"></td>
                </tr><br>
            `;
                }
                showTeam.innerHTML += `</table><button onclick="accept()" id="accept" class="accept btn_effect">Accept</button>`
            };
            if (select.value == 'f12010') {
                riders.splice(0, riders.length);
                const team_qty = f1_10.length;
                showTeam.innerHTML += `<input type="button" value="${team_qty}" id="team_qty">
                <input type="text" placeholder="Team" readonly>
                <input type="number" placeholder="Tec" readonly>
                <input type="number" placeholder="Con" readonly>
                ...
                for (let i = 0; i < team_qty; i++) {
                    ...
                    showTeam.innerHTML += `<tr>
               <td> <input type="text" value="${f1_10[i].name}" id="rider${i}_name"> </td>
               <td> <input type="text" value="${f1_10[i].team}" id="rider${i}_team"> </td>
               <td><input type="number" value="${f1_10[i].tech}" id="rider${i}_tech" max="99" min="1"> </td>
                <td><input type="number" value="${f1_10[i].con}" id="rider${i}_con" max="99" min="1"> </td>
                    <td><input type="number" value="${f1_10[i].eng}" id="rider${i}_eng" max="99" min="1"></td>
                </tr><br>
            `;
                }
                showTeam.innerHTML += `</table><button onclick="accept()" id="accept" class="accept btn_effect">Accept</button>`
            };
            if (select.value == 'f12008') {
                riders.splice(0, riders.length);
                const team_qty = f1_08.length;
                showTeam.innerHTML += `<input type="button" value="${team_qty}" id="team_qty">
                <input type="text" placeholder="Team" readonly>
                <input type="number" placeholder="Tec" readonly>
                <input type="number" placeholder="Con" readonly>
                ...
                for (let i = 0; i < team_qty; i++) {
                    ...
                    showTeam.innerHTML += `<tr>
               <td> <input type="text" value="${f1_08[i].name}" id="rider${i}_name"> </td>
               <td> <input type="text" value="${f1_08[i].team}" id="rider${i}_team"> </td>
               <td><input type="number" value="${f1_08[i].tech}" id="rider${i}_tech" max="99" min="1"> </td>
                <td><input type="number" value="${f1_08[i].con}" id="rider${i}_con" max="99" min="1"> </td>
                    <td><input type="number" value="${f1_08[i].eng}" id="rider${i}_eng" max="99" min="1"></td>
                </tr><br>
            `;
                }
                showTeam.innerHTML += `</table><button onclick="accept()" id="accept" class="accept btn_effect">Accept</button>`
...
function play(rider, laps,r_id){
 ...
 let time = 0;
    for(let i = 0; i < laps; i++){
        ...
        let trand = Math.floor(Math.random()*20);
        let trnd = Math.floor(Math.random()*20); //+/-
        let tech_r = 0;
        trnd<5 ? tech_r-=trand : tech_r=trand;

        let erand = Math.floor(Math.random()*20);
        let ernd = Math.floor(Math.random()*20); //+/-
        let eng_r = 0;
        ernd<5 ? eng_r-=erand : eng_r=erand;
...
<tr id="r2"><td colspan="4"><button onclick="play(riders[0],57,r2), play(riders[1],57,r2),play(riders[2],57,r2),play(riders[3],57,r2),play(riders[4],57,r2),play(riders[5],57,r2),play(riders[6],57,r2),play(riders[7],57,r2),play(riders[8],57,r2),play(riders[9],57,r2),play(riders[10],57,r2),play(riders[11],57,r2),play(riders[12],57,r2),play(riders[13],57,r2),play(riders[14],57,r2),play(riders[15],57,r2),play(riders[16],57,r2),play(riders[17],57,r2),play(riders[18],57,r2),play(riders[19],57,r2)" class="play_button btn_effect">Race Start</button></td>
</tr>
<tr>
    <th colspan="4" class="end_round">X</th>
</tr>
